...
public class Lesson50 : MonoBehaviour
{
    ...
    private void MatchTarget()   //在动画中添加该事件保证调用该函数时动画已经切换
    {
        //参数一:目标位置
        //参数二:目标角度
        //参数三:匹配的骨骼位置
        ...
        //参数五:开始位移动作的百分比
        //参数六:结束位移动作的百分比
        animator.MatchTarget(target.position, target.rotation, AvatarTarget.LeftFoot,new MatchTargetWeightMask(Vector3.one,1),0.4f,0.64f);
    }
}
