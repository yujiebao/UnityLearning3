...
public class Lesson47 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        #region 知识点二 2D混合的种类
        // 2D Simple Directional    --2D简单定向模式  运动表示不同方向时使用  比如向前、后、左、右走   同一方向只能有一种(例如：只能走或者只能跑)
        // 2D Freeform Directional  --2D自由形式定向模式 同上 运动表示不同方向时使用 但是可以在同一方向上运动 比如向前跑和走 同一方向可以有多种运动(例如：走和跑)
